...
void write_block_bitmap(int fd)
{
	...
	for(int i = 0; i < BLOCK_SIZE; i++) {
		...
		if (i > 2 && i < 127) {
			// for i in [24, 1024), block them as free
			map_value[i] = 0x00;
		} else {
			// for else part, mark all non-existing block as '1'
			map_value[i] = 0xff;
		}
		map_value[2] = 0x7f;
		map_value[127] = 0x80;
...
void write_inode_bitmap(int fd)
{
	...
	// for i in [2, 128), block them as free
	for (int i = 2; i < BLOCK_SIZE/64 - 1; i++) {
		map_value[i] = 0x00;
	}

	// for else part, mark all remaining block as '1'
	for (int i = BLOCK_SIZE/64; i < BLOCK_SIZE; i++) {
		map_value[i] = 0xff;
	}
...
void write_inode_table(int fd) {
	...
	root_directory.i_mode=EXT2_S_IFDIR
							...
							| EXT2_S_IROTH
							| EXT2_S_IXOTH;
	root_directory.i_uid = 0;
	root_directory.i_size = BLOCK_SIZE;
    ...
    // Case 2: hello_world
	// Ownership: uid 1000 and gid 1000
	// Permission flag: 
	...
	memcpy(hello_inode.i_block, "hello-world", 12); 
...
void write_root_dir_block(int fd)
{
	...
	bytes_remaining -= parent.rec_len;

	// for lost_and_found entry

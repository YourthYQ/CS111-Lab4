...
void write_inode_table(int fd) {
	...
	struct ext2_inode hello_sym={0};
	hello_sym.i_mode=EXT2_S_IFLNK
						| EXT2_S_IRUSR
						| EXT2_S_IWUSR
						| EXT2_S_IRGRP
						| EXT2_S_IROTH;
	hello_sym.i_uid=1000;
	hello_sym.i_size=11;
	hello_sym.i_atime=current_time;
	hello_sym.i_ctime=current_time;
	hello_sym.i_mtime=current_time;
	hello_sym.i_dtime=0;
	hello_sym.i_gid=1000;
	hello_sym.i_links_count=1;
	hello_sym.i_blocks=0;
	memcpy(hello_sym.i_block, "hello-world", 12); 
	write_inode(fd, HELLO_INO, &hello_sym);
}
